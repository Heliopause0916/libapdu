...
```mermaid
flowchart TD
    A[输入字节序列 buf] --> B{长度检查 >= 4 ?}
    B -->|否| C[返回错误]
    B -->|是| D[解析 CLA INS P1 P2]
    D --> E{剩余长度 == 0 ?}
    E -->|是| F[Case 1]
    E -->|否| G[检查首字节]
    G --> H{首字节 = 0x00<br/>且 len >= 3 ?}
    H -->|是| I[扩展 APDU]
    H -->|否| J[短 APDU]
    I --> K[解析 3 字节长度字段]
    J --> L[解析 1 字节长度字段]
    K --> M[判断 Case 2/3/4 Ext]
    L --> N[判断 Case 2/3/4 Short]
```
...
```mermaid
flowchart TD
    A[apdu_t 结构体] --> B[写入 CLA INS P1 P2]
    B --> C[根据 cse 分发]
    C --> D[Case 1]
    C --> E[Case 2]
    C --> F[Case 3]
    ...
    C --> H[其他]
    D --> I["T0: +0x00<br/>T1: 无"]
    E --> J["Short/Ext<br/>根据协议"]
    F --> K["Lc + Data<br/>根据协议"]
    G --> L["Lc + Data + Le<br/>根据协议"]
    H --> M[错误]
```
